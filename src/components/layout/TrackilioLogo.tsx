...
export function TrackilioLogo({ className = '', showText = true, size = 46 }: TrackilioLogoProps) {
  return (
    <Link href="/" className={`inline-flex items-center gap-2.5 group shrink-0 ${className}`}>
      ...
      {showText && (
        <span className="font-sans text-2xl font-black tracking-tight text-[#222222] group-hover:text-[#FA8112] transition-colors">
          Trackilio
        </span>
      )}
    </Link>
